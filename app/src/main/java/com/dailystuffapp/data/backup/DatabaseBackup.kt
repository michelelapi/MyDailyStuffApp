...
class DatabaseBackup(private val context: Context) {
        ...
        companion object {
                 ...
                 * Activity and use startActivityForResult() or registerForActivityResult() to
                 * handle the sign-in.
                 */
                fun getSignInIntent(context: Context): android.content.Intent {
                        val signInOptions =
                                GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN)
                                        ...
                                        .requestProfile() // Request profile (helps get email on
                                        // some devices)
                                        .build()
                ...
                /**
                 * Signs out from Google Sign-In. Call this if you need to force re-sign-in to
                ...
                withContext(Dispatchers.IO) {
                        try {
                                ...
                                if (lastBackupDate == today) {
                                        Log.d(
                                                TAG,
                                                "Backup already performed today ($today). Skipping..."
                                        )
                                ...
                                Log.d(
                                        TAG,
                                        "Fetched ${tasks.size} tasks and ${completions.size} completions"
                                )
                                ...
                                Log.d(
                                        TAG,
                                        "Backup completed successfully. Saved backup date: $today"
                                )
                                Result.success(Unit)
                        } catch (e: Exception) {
                        ...
                        try {
                                ...
                                val signInOptions =
                                        GoogleSignInOptions.Builder(
                                                ...
                                                .requestScopes(Scope(DriveScopes.DRIVE))
                                                .requestEmail() // Request email access
                                                .requestProfile() // Request profile (helps get
                                                // email on some devices)
                                                .build()
                                ...
                                // Get account email - try multiple sources
                                // Priority: account.email > account.account?.name > account.id
                                // This is more reliable on real devices where email might be null
                                val accountEmail =
                                        account.email ?: account.account?.name ?: account.id

                                // Log all available account information for debugging
                                Log.d(
                                        TAG,
                                        "Account info - email: ${account.email}, " +
                                ...
                                if (accountEmail.isNullOrBlank()) {
                                        throw IllegalStateException(
                                                "Unable to get account email. " +
                                                        "Please sign out and sign in again:\n" +
                                                        "1. Go to Settings > Apps > MyDailyStuffApp\n" +
                                                        "2. Clear app data or uninstall/reinstall\n" +
                                                        "3. Open the app and sign in again\n" +
                                                        "4. Make sure to grant email permission when prompted"
                                        )
                                }

                                Log.d(TAG, "Using Google account: $accountEmail")
                                ...
                                Log.d(
                                        TAG,
                                        "Credential created for account: ${credential.selectedAccountName}"
                                )
